...
from .components import get_query_all, get_users_name
...
@route.get("/get_user_name", summary="获取所有用户名称")
...
@route.post("/save_uid_doc_permissions", summary="保存用户阅读权限")
async def save_uid_doc_permissions(request: Request, docId: str = Query(None)):
    """
    通过传入工作流实例wid等信息创建实际的任务实例记录
    :param request:
    :param db: 数据库会话
    ...
    callback_json = constructResponse()
    callback_json.statusCode = 400
    doc_permissions = json.loads(data.get('doc_permissions', ''))
    content = {}
    pprint(data)
    print(docId)
