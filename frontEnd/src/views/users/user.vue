...
import {edit_person} from '~/api/account';
...
const onSubmit = async () => {
  // 向后端发起修改操作
  const response = (await edit_person(editForm));
  if (response.isSuccess) {
    // 响应成功则弹出提示
    ElMessage.success(name+',修改成功！重新登录..');
    // 清理本地缓存数据
    localStorage.clear();
    ...
    ElMessage.error(name+',修改失败！'+response.errMsg);
  }
};
